// Hero section for the Cardiac Calcium Scoring (CAC) page.
// Note: uses z-0 instead of z-10 used on all other service page heroes.
// The Experience section here uses z-10 (not z-20), so the relative stacking
// is preserved — just one level lower than the other service pages.
import { ArrowRight } from "lucide-react";
import heroImage from "../../assets/cardiac-scoring/cardiac.jpg";
import { DotGrid, Rings, SoftCircle } from "./Decorations";

const Hero = () => {
  return (
    <section
      // sticky + z-0: stays pinned while the user scrolls until the Experience section
      // slides on top. z-0 (vs z-10 on other pages) because the Experience section
      // here is z-10 (not z-20), so relative ordering is still correct.
      ...
      style={{
        ...
        // while the right side reveals the cardiac background photo.
      ...
      <div className="relative z-10 w-full flex flex-col items-start justify-center gap-6 px-4 md:px-8 lg:px-16 xl:px-32 2xl:px-60 pb-16 pt-14 sm:pb-20 md:gap-8">
        <div className="max-w-xl w-full sm:max-w-2xl">
          ...
          <p className="mt-5 max-w-md text-base leading-relaxed text-[#2a4a6e] sm:mt-6 sm:text-lg">
            Experience preventive heart screening with advanced CT Imaging – a fast, non-invasive CT scan that gives you and your physician a clearer picture of your cardiac health.
          </p>
